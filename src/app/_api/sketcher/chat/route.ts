...
function systemPrompt(input: z.infer<typeof requestSchema>): string {
  return [
    ...
    '',
    'IMPORTANT: When the user asks you to build, create, make, or design a page/screen/UI,',
    'respond with ONLY a ```html code block containing a complete standalone HTML page.',
    'Include all CSS in a <style> tag (no external CDN). Make it professional with modern design.',
    'Do NOT add any text before or after the code block — no explanation, no description,',
    'no "here is your page", just the raw ```html block. The system auto-saves it as a file.',
    '',
